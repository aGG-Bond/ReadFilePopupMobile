...
class FilePreview {
  constructor(options) {
     ...
     * @param {Object} options.listObj - 文件列表
     *
     */
    // 默认配置
    this.Configns = Object.assign(
      {
        ...
        coerceReadList: {
          ...
          coerceCallBack: [
            (control, index) => {
              console.log("btn[0]强制阅读弹窗结果：", control, index);
            },
            (control, index) => {
              console.log("btn[1]强制阅读弹窗结果：", control, index);
            },
          ],
        },
  ...
  initModal() {
    ...
    isBindFileClick && this.bindFileClick(listObj.listId);
  }

  // 数据转换
  dataChange(data) {
    if (!this.Configns.fileKeyNameConfign)
      return console.error("请传入文件keyNameConfign");
    ...
    if (!fileTitle || !filePdfUrl || !fileRichContent || !fileArr)
      return console.error("请传入文件keyNameConfign的参数");
  ...
  drawReadFileList(listObj) {
    ...
    if (!ID) return console.error("请传入需要添加的dom ID");
    if (fileList.length < 1) return console.error("请传入需要渲染的文件数据");

    let html = `${
      isCheckButton ? `<input type="checkbox" id="${checkButtonID}" />` : ""
    }${listText}`;
    ...
    for (let i = 0, len = fileList.length; i < len; i++) {
      ...
      html += `<span class="pdfsee item-contract" data-pdf="${
        file_type == 2 ? pdf_url : ""
    ...
    if (!element) return console.error("未找到元素", ID);
  ...
  // 富文本文件
  richTextFile({ file: filebox, fromChooseList = false, isCoerce = false }) {
    ...
    popupInstance.showBottomPopup({
      ...
      // btns: ['确定'],
      callbacks: function (index) {
        console.log("richTextFile callback", index);
      },
    });
  }

  // 多分pdf选择
  choosePdfFile({ file: filebox, fromChooseList = false, isCoerce = false }) {
    ...
    popupInstance.showBottomPopup({
      ...
      // btns: ['确定'],
      callbacks: function (index) {
        console.log("choosePDffile callback");
      },
    });
    this.bindFileClick(".ChoosePdfFileList", fileArr);
    // $('.con-list1').append(html);
    // ifFixed(true);
  }
  ...
  bindFileClick(containerSelector, fileArr) {
    ...
    if (!container) return console.error("容器未找到:", containerSelector);
  ...
  // 创建按钮回调函数
  createButtonCallbacks(btnArr, coerceCallBack, control) {
    const callbacks = [];
    ...
    if (
      ...
      if (typeof showProgressInButton === "number") {
        ...
        if (buttonIndex >= 0 && buttonIndex < finalButtons.length) {
          finalButtons[
            buttonIndex
          ] = `${btnArr[buttonIndex]}(${currentDisplayIndex}/${totalLength})`;
        }
  ...
  openCoerceReadPopup() {
    ...
    console.log("coerceCallBack", this.Configns.coerceReadList);

    if (fileList.length === 0) return;
    // 创建设置复选框的函数
    const setCheckboxChecked = (isChecked) => {
    ...
    const showNextFile = (btnTitleArr) => {
      ...
      // 确保文件类型已设置
      if (!file.file_type) {
        file.file_type = this.judgeFileType({ type: "", file, isCoerce: true });
      }
      ...
      const control = {
        ...
        isLastFile: () => currentIndex === fileList.length - 1, // 是否为最后一份文件
      ...
      // 为每个按钮创建回调函数
      const callbacks = this.createButtonCallbacks(
        btnArr,
        coerceCallBack,
        control
      );
      ...
      // 处理按钮标题
      const finalButtons = this.processButtonTitles(
        btnArr,
        customButtonTitles,
        fileList,
        currentIndex,
